...
# 兼容仓库内自带的 SecureINT/p4utils 目录结构。
THIS_DIR = os.path.dirname(os.path.abspath(__file__))
SECUREINT_BMV2_DIR = os.path.join(THIS_DIR, 'SecureINT-main', 'BMv2')
if SECUREINT_BMV2_DIR not in sys.path:
    sys.path.insert(0, SECUREINT_BMV2_DIR)
...
def get_args():
    ...
    parser.add_argument('--p4', help='p4 src file.',
                        type=str, required=False, default='ltd.p4')
...
def main():
    args = get_args()
    if not os.path.isfile(args.p4):
        raise FileNotFoundError('P4 source not found: {}'.format(args.p4))
